...
#if !defined(HAVE_CLOCK_GETTIME) || !defined(HAVE_SNPRINTF) || !defined(HAVE_RANDOM)
#include "compat.h"
#endif
...
//    m_interval_timestamp = time((time_t*) 0);
    m_interval_timestamp = ttime((time_t*) 0);
...
size_t PeerList::What_Can_Duplicate(BitField &bf, const btPeer *proposer, size_t idx)
{
  struct qdata {
...
//    rightnow = nowspec.tv_sec + (double)(nowspec.tv_nsec)/1000000000;
    rightnow = nowspec.ts_sec + (double)(nowspec.ts_nsec)/1000000000;
...
//acw
//    rightnow = nowspec.tv_sec + (double)(nowspec.tv_nsec)/1000000000;
    rightnow = nowspec.ts_sec + (double)(nowspec.ts_nsec/1000000000.0);

...
void ConStream::Associate(FILE *stream, const char *name, int mode) /*fold00*/
{
  m_stream = stream;
  m_filemode = (UCHAR)mode;
...
int ConStream::SameDev(ConStream *master) const /*fold00*/
{
...
int ConStream::IsTTY() const /*fold00*/
{
  return (Fileno() >= 0) ? isatty(Fileno()) : 0;
}


void ConStream::PreserveMode() /*fold00*/
{
...
void ConStream::RestoreMode() /*fold00*/
{
...
void ConStream::SetInputMode(int keymode) /*fold00*/
{

  if( m_suspend ) return;

  m_inputmode = (UCHAR)keymode;
  if( !IsTTY() ) return;
...
int ConStream::Output(const char *message, va_list ap) /*fold00*/
{
...
int ConStream::Output2(char *buf)  //acw /*fold00*/
{
  if( m_suspend ) return 0;

  int old_newline = m_newline;
  _newline();
  StLineOut(m_stream, buf);
  fprintf(m_stream, "\n");
  fflush(m_stream);
  return (old_newline==m_newline) ? 0 : 1;
}

int ConStream::Output_n(const char *message, va_list ap) /*fold00*/
{
  if( m_suspend ) return 0;
...
int ConStream::Output_n2(char *buf) /*fold00*/
{
  if( m_suspend ) return 0;
...
int ConStream::Update(const char *message, va_list ap) /*fold00*/
{
...
int ConStream::Update2(const char *format, char *buf)  //acw /*fold00*/
{
...
char *ConStream::Input(char *field, size_t length) /*fold00*/
{
...
//  return fgets(field, length, m_stream);
  ...
  while(1) {
    ...
    if(!(KbdKeyInfo.fbStatus & 1<<1) && !(KbdKeyInfo.fsState & 1<<2) &&  // символ и не ctrl.
      (KbdKeyInfo.chChar >= 0x20 || KbdKeyInfo.chChar == 0x08 || KbdKeyInfo.chChar == 0x0D)) {
      if(KbdKeyInfo.chChar == 0x0D) {
        *pPtr = '\0';
	break;
      }
      if(KbdKeyInfo.chChar == 0x08) {
	if(Cnt) {
	  printf("\b \b");
	  fflush(stdout);
	  --Cnt;
      ...
      if(Cnt <= length - 2) {
        *pPtr++ = KbdKeyInfo.chChar;
        ++Cnt;
        printf("%c", KbdKeyInfo.chChar);
        fflush(stdout);
      }
    }
  }
...
inline void ConStream::_newline() /*fold00*/
{
  if( !m_newline ){
    fprintf(m_stream, "\n");
    m_newline = 1;
...
inline int ConStream::_convprintf(const char *format, va_list ap) /*fold00*/
{
  int r = ( '\n' == format[strlen(format)-1] );
  m_newline = (UCHAR)r;
...
//acw
//  m_status_format = 0;
  m_status_format = f_clStyle;

  int i = 0;
  m_statusline[i++] = &Console::StatusLine0;
  m_statusline[i++] = &Console::StatusLine1;
  m_statusline[i++] = &Console::StatusLine2;
  m_statusline[i++] = &Console::StatusLine3;
  if( STATUSLINES > i ){
...
int Console::IntervalCheck(void) /*fold00*/
...
int Console::ChangeChannel(int channel, const char *param, int notify) /*fold00*/
{
  ...
  if( 0==strcasecmp(param, m_stdout.GetName()) ) dest = &m_stdout;
    ...
    if( !dest ){
      ...
      if(dest) {
        ...
        stream = fopen(param, (channel==O_INPUT) ? "rb" : "ab");
        if(stream)
...
void Console::Status(int immediate) /*fold00*/
{
...
//      Update("%*s", -(int)sizeof(buffer)+1, buffer);
      ...
      APIRET  rc;
      rc = DosRequestMutexSem(hmtxStLine, SEM_IMMEDIATE_RETURN);
      if(rc == NO_ERROR) {
	m_status_format = f_clStyle;
//	(CONSOLE.*m_statusline[m_status_format])(buffer, sizeof(buffer));
//dif	(CONSOLE.*m_statusline[m_status_format])(buffer);
...
void Console::StatusLine0(char buffer[]) /*fold00*/
...
//acw
  ...
  ULONGLONG  ullUp = (ULONGLONG)(Self.TotalUL() >> 20);

  fRateDwn = (float)(Self.RateDL() / 1024.0);
  fRateUp = (float)(Self.RateUL() / 1024.0);

  int  PreDL = (int)(m_pre_dlrate.RateMeasure(Self.GetDLRate()) >> 10);
  int  PreUL = (int)(m_pre_ulrate.RateMeasure(Self.GetULRate()) >> 10);
...
//    "%c %d/%d/%d [%d/%d/%d] %lluMB,%lluMB | %d,%dK/s | %d,%dK E:%d,%d %s%s",
    "%c %d/%d/%d [%d/%d/%d] %.0fMB,%lluMB | %.0f,%.0fK/s | %d,%dK E:%d,%d %s%s",
...
//    sprintf(pStLine, "%d/%d/%d [%d/%d/%d] %lluMB,%lluMB | %d,%dK/s | %d,%dK E:%d,%d %s",
    sprintf(pStLine, "%d/%d/%d [%d/%d/%d] %.0fMB,%lluMB | %.0f,%.0fK/s | %d,%dK E:%d,%d %s%s",
...
void Console::StatusLine1(char buffer[]) /*fold00*/
...
//acw
  ...
  int  int1 = PeerSeed;
  int  int2 =  (int)(Tracker.GetSeedsCount()) - (BTCONTENT.IsFull() ? 1 : 0);

  int  int3 = (int)(WORLD.GetPeersCount()) - WORLD.GetSeedsCount() - WORLD.GetConnCount();
  int  int4 = (int)(Tracker.GetPeersCount()) - Tracker.GetSeedsCount() -
              (!BTCONTENT.IsFull() ? 1 : 0);

  int  int5 = (int)(WORLD.GetConnCount());

  double  d1 = (double)(Self.TotalUL()) / ( Self.TotalDL() ? Self.TotalDL() :
                BTCONTENT.GetTotalFilesLength() );

  int  int6 = (int)(fRateDwn);
  int  int7 =  (int)(fRateUp);
...
//acw
/*
    int1, int2, int3, int4, int5, d1, int6, int7,
...
//    "%c S:%d/%d L:%d/%d C:%d  R=%.2f D=%d U=%d K/s  %s %s  %s%s",
    ...
    sprintf(pStLine, "S:%d/%d L:%d/%d C:%d  R=%.2f D=%d U=%d K/s  %s %s  %s",
            int1, int2, int3, int4, int5, d1, int6, int7,
            complete, timeleft,
            partial);
...
void Console::StatusLine2(char buffer[]) //acw /*fold00*/
{
  StLine2(buffer);
}


void Console::StatusLine3(char buffer[]) //acw /*fold00*/
{
  StLine3(buffer);
}


void Console::Print(const char *message, ...) /*fold00*/
{
  va_list ap;
...
*/
void Console::Print_n(const char *message, ...) /*fold00*/
{
  va_list ap;
...
void Console::Update(const char *message, ...) /*fold00*/
...
void Console::Update2(const char *message, char *buf)  // acw /*fold00*/
/* Update (replace) the current line (no terminating newline) */
{
  ...
  if( K_LINES != m_streams[O_INPUT]->GetInputMode() ||
      ...
      (!m_streams[O_NORMAL]->SameDev(m_streams[O_INTERACT]) &&
       !m_streams[O_NORMAL]->SameDev(m_streams[O_INPUT])) ){
    if( m_streams[O_NORMAL]->Update2(message, buf) )
      SyncNewlines(O_NORMAL);
  }
  if( arg_verbose && !m_streams[O_DEBUG]->SameDev(m_streams[O_NORMAL]) ){
    if( m_streams[O_DEBUG]->Update2(message, buf) )
      SyncNewlines(O_DEBUG);
  }
}


void Console::Warning(int sev, const char *message, ...) /*fold00*/
...
void Console::Debug(const char *message, ...) /*fold00*/
{
  static char buffer[80];
  ...
  if( K_LINES != m_streams[O_INPUT]->GetInputMode() ||
    ...
    format = new char[need];
    if(need > sizeof(buffer) && format) buflen = need;
...
void Console::Debug_n(const char *message, ...) /*fold00*/
...
void Console::Interact(const char *message, ...) /*fold00*/
{
  va_list ap;
...
void Console::Interact2(char *buf) /*fold00*/
{

  if( m_streams[O_INTERACT]->Output2(buf) )
    SyncNewlines(O_INTERACT);

}

void Console::Interact_n(const char *message, ...) /*fold00*/
...
void Console::Interact_n2(char *buf) /*fold00*/
/* Print interactive message without a terminating newline
   With a null message, start/insure a new line */

{
  if( m_streams[O_INTERACT]->Output_n2(buf) )
    SyncNewlines(O_INTERACT);
}

void Console::InteractU(const char *message, ...) /*fold00*/
...
char *Console::Input(const char *prompt, char *field, size_t length) /*fold00*/
...
void Console::SyncNewlines(int master) /*fold00*/
{
  for( int i=0; i < O_NCHANNELS; i++ ){
...
//      (unsigned long)(time((time_t *)0) - BTCONTENT.GetStartTime()),
      (unsigned long)(ttime((time_t *)0) - BTCONTENT.GetStartTime()),
      clock() / (double)CLOCKS_PER_SEC /
//acw t
//        (time((time_t *)0) - BTCONTENT.GetStartTime()) * 100 );
        (ttime((time_t *)0) - BTCONTENT.GetStartTime()) * 100 );
}

int Console::UserMenu(void) /*fold00*/
{
...
//  if(m_streams[O_INPUT]->Fileno() >= 0) {
	...
	if( '0'==pending ){
	  ...
	  case 'n':				// get1file
	    ...
	    if(f_clFile && arg_file_to_download) {
	      if(pArgFiles) delete []pArgFiles;
	      pArgFiles = new char[strlen(param) + 2];
	      strcpy(pArgFiles, param);
//acw
	      if(ParsFileNums(arg_file_to_download, pArgFiles)) {
//		CONSOLE.Warning(1, "\nincorrect file list format");
		Warning(1, "\nincorrect file list format");
	      }
	      else {
//                ChkFileNums(pFiles);
//                if(BTFILESH.CreateFiles() == -1) exit(-1);
...
*/
	Interact(" %-9s%-30s %-9s%s", "X", "Completion command",
        ...
        Interact(" %-9s%-30s %-9s%s", "E[+/-]", "Adjust seed exit ratio",
          "Q", "Quit");
	if(uFilesMax > 1) {  // есть из чего выбирать.
	  Interact(" %-9s%-30s %-9s%s", "n", "Download specific files",
	    "", "");
	}
	break;
	...
	if(uFilesMax > 1) {  // есть из чего выбирать.
	    ...
	    m_streams[O_INPUT]->SetInputMode(K_LINES);
	    ShowFiles();
            Interact("Enter * for all files (normal behavior).");
//        if( arg_file_to_download )
            if( arg_file_to_download && *arg_file_to_download)
	...
	case 'd': RequestMutex();
	  max_bandwidth_down +=
            ( (max_bandwidth_down * (abs(inc)/100.0) < 1) ? inc :
                (int)(max_bandwidth_down * (inc/100.0)) );
	  if( max_bandwidth_down < 0 ) max_bandwidth_down = 0;
	  f_max_bandwidth_down = 1;
//	  ReleaseMutex();
          break;
	case 'u': RequestMutex();
	  max_bandwidth_up +=
            ( (max_bandwidth_up * (abs(inc)/100.0) < 1) ? inc :
                (int)(max_bandwidth_up * (inc/100.0)) );
          if( max_bandwidth_up < 0 ) max_bandwidth_up = 0;
	  f_max_bandwidth_up = 1;
//	  ReleaseMutex();
          break;
	case 'e': RequestMutex();
	  value= (int)seed_hours;
	  value += (value / 100.0 < 1) ? step : (step * value / 50.0);
	  if(value < 0) value = 0;
	  else if(value > 1000 * 1000) value = 1000 * 1000;
	  seed_hours = value;
	  f_seed_hours = 1;
//	  ReleaseMutex();
          break;
	case 'E': RequestMutex();
	  if(seed_ratio <= 10.0 && step < 0 || seed_ratio <= 9.9)
	    seed_ratio += step / 10.0;
	  else seed_ratio += step;
	  if( seed_ratio < 0 ) seed_ratio = 0;
	  f_seed_ratio = 1;
//	  ReleaseMutex();
          break;
	case 'm': RequestMutex();
	  value = (int)min_peers; value += step;
          min_peers = (value < 1) ? 1 : (size_t)value;
	  if( min_peers > max_peers ) min_peers = max_peers;
	  f_min_peers = 1;
//	  ReleaseMutex();
          break;
	case 'M': RequestMutex();
	  value = (int)max_peers; value += step;
          max_peers = (value < (int)min_peers) ? min_peers : (size_t)value;
	  if( max_peers > 1000 ) max_peers = 1000;
	  f_max_peers = 1;
//	  ReleaseMutex();
          break;
	case 'C': RequestMutex();
	  value = (int)cache_size; value += step;
          cache_size = (value < 0) ? 0 : (size_t)value;
...
//          else CTCS.Send_Config();
	  if(!f_Mutex) RequestMutex();
	  if( 'd'==pending || 'u'==pending ) f_CTCSRq = 1;
	  else f_CTCSRq = 2;
        }
	if(f_Mutex) ReleaseMutex();
	break;
...
//        Status(1);
      ...
      switch( pending ){
      case 'd': InteractU("DL Limit: %d B/s ", (int)max_bandwidth_down);
        break;
      case 'u': InteractU("UL Limit: %d B/s ", (int)max_bandwidth_up);
        break;
/*
      case 'e': InteractU("Seed time: %.0f hours ", BTCONTENT.GetSeedTime() ?
//acw dt
//          (cfg_seed_hours - (now - BTCONTENT.GetSeedTime())/(double)3600) :
          (cfg_seed_hours - (now - BTCONTENT.GetSeedTime())/(double)3600) :
          (double)cfg_seed_hours);
*/
      case 'e': InteractU("Seed time: %u hours ", seed_hours);
        break;
      case 'E': InteractU("Seed ratio: %.1f ", (double)seed_ratio);
        break;
      case 'm': InteractU("Minimum peers: %d ", (int)min_peers);
        break;
      case 'M': InteractU("Maximum peers: %d ", (int)max_peers);
        break;
      case 'C': InteractU("Maximum cache: %d MB ", (int)cache_size);
        break;
      default:
        break;
      }
...
int Console::OperMenu(const char *param) /*fold00*/
// Return non-zero to exit operator menu mode.
{
  ...
  if( 0==oper_mode ){
    Interact("Operator Menu (ENTER for return)");
    n_opt = 0;
...
//    char buffer[80];
    ...
    for( int i=0; i < STATUSLINES; i++ ){
      int Len = sprintf(buffer, " %c%d) ",(i==m_status_format) ? '+' : ' ', ++n_opt);
//      (CONSOLE.*m_statusline[i])(buffer + Len, sizeof(buffer));
//dif      (CONSOLE.*m_statusline[i])(buffer + Len);
//      (this->*m_statusline[i])(buffer, sizeof(buffer));
      (this->*m_statusline[i])(buffer + Len);
      Interact2(buffer);
    }
...
//acw    if( !arg_daemon )
//acw      Interact(" %2d) Become daemon (fork to background)", ++n_opt);
...
//acw
      f_clStyle = (UCHAR)m_status_format;
...
//      Interact("Ratio: %.2f   Seed time: %luh   Seed ratio: %.2f",
      Interact("Ratio: %.1f   Seed time: %luh   Seed ratio: %.1f",
        (double)(Self.TotalUL()) / ( Self.TotalDL() ? Self.TotalDL() :
                                     BTCONTENT.GetTotalFilesLength() ),
...
*/
    ...
    else if( sel == 3 + O_NCHANNELS+1 + STATUSLINES ){  // update tracker
...
void Console::RequestMutex(void)  //acw /*fold00*/

{
  DosRequestMutexSem(hmtxConsole, SEM_INDEFINITE_WAIT);
  f_Mutex = 1;
}

void Console::ReleaseMutex(void)  //acw /*fold00*/
{

...
int btContent::PrintOut()
{
  ...
  if( m_create_date ){
    ...
    char *c;
    c = ctime(&m_create_date);

    if( c[strlen(c)-1] == '\n' ) c[strlen(c)-1] = '\0';
    CONSOLE.Print("Created On: %s", c);
  }
...
//acw
      if(CSC(m_comment, (char*)s, r + 1, r, 'u')) {  // перекодировалось неуспешно.
...
//acw
  ulChunksMax = m_npieces;
  ulChunkLen = m_piece_length;
  if(uFilesMax == 1) {  // всего 1 файл.
    f_clFile = 0;
    if(arg_file_to_download) {
      arg_file_to_download = NULL;
    }
  }
  if(f_clFile) {
    if(ParsFileNums(arg_file_to_download, pArgFiles)) {
      printf("\n! '-file' option has incorrect argument");
      exit(-1);
    }
    ChkFileNums(pFiles);
  }
...
//acw
  if(f_clFile) ChkChunks(pFileExist);
  ...
  if( !m_cache_size ) return buf ? m_btfiles.IO(buf, offset, len, 0) : 0;
    ...
    while( len && p ){
      ...
      if( offset < p->bc_off ){
        ...
        char *src;
        if( offset > p->bc_off ){
          len2 = (size_t)(p->bc_off + p->bc_len - offset);
...
void btContent::CacheConfigure()
{
  if( cfg_cache_size ){
    if( cfg_cache_size > GetTotalFilesLength()/1024/1024 )
      cfg_cache_size = (UINT)((GetTotalFilesLength()+1024*1024-1)/1024/1024);
    CacheEval();
  }else m_cache_size = 0;
...
void btContent::FlushQueue()
{
  ...
  if (!NeedFlush() && m_need_readonly)  //nickk
  {
    m_btfiles.setReadOnly();
    m_need_readonly = 0;
  }
}
...
ssize_t btContent::CacheIO(char *buf, uint64_t off, size_t len, int method)
{
  ...
  size_t idx = (size_t)(off / m_piece_length);
  p = m_cache[idx];
  if( p ) pp = p->bc_prev;
...
//acw
    if(!f_clFile || (f_clFile && pChunkChk[idx + 1])) {
       ++Chk;
...
//acw
      if(!f_clFile) CONSOLE.InteractU("Check exist: %d/%d", idx+1, m_npieces);
      else CONSOLE.InteractU("Check exist: %d/%d", Chk, ulChunkChk);
    }
  }
...
//        if( (dash = strchr(tok, '-')) && (!plus || dash < plus) ){  //acw
	dash = strchr(tok, '-');
